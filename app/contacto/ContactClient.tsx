...
export default function ContactPageClient({ products }: Props) {
  ...
  const product = useMemo(
    () => products.find((p) => p.slug === selected),
    [products, selected]
  );
    ...
    <Section className="py-[var(--space-12)]">
      <div className="mx-auto grid max-w-6xl grid-cols-1 gap-8 md:grid-cols-5">
        ...
        <div className="md:col-span-3">
          <h1 className="font-display mb-2 text-3xl md:text-4xl">
            Solicita tu producto Synko
          </h1>
          <p className="text-muted mb-6">
            Selecciona el producto, añade add-ons y cuéntanos tu caso.
          </p>
            ...
            <div aria-live="polite" className="mb-4 min-h-6 text-sm">
              {state?.ok && (
                <p className="text-[var(--color-accent)]">
                  ¡Gracias! Te contactamos en &lt;24h laborables.
                </p>
              )}
              {!state?.ok && state?.error && (
                <p className="text-[var(--color-highlight)]">{state.error}</p>
              )}
            </div>
            ...
            <form action={action} noValidate className="grid grid-cols-1 gap-4">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  ...
                  <input
                    ...
                    name="name"
                    required
                    className="bg-bg mt-1 w-full rounded-lg border border-[var(--color-border)] px-3 py-2"
                  />
                  {state?.errors?.name && (
                    <p className="mt-1 text-xs text-[var(--color-highlight)]">
                      {state.errors.name[0]}
                    </p>
                  )}
                </div>
              ...
              <div>
                ...
                {product?.priceHint && (
                  <p className="mt-1 text-xs text-[var(--color-muted)]">
                    Precio orientativo:{" "}
                    <span className="font-medium text-[var(--color-highlight)]">
                      {product.priceHint}
                    </span>
                  </p>
                )}
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {product.addons.map((a) => (
                      <label
                        key={a}
                        className="inline-flex items-center gap-2 rounded-lg border border-[var(--color-border)] px-3 py-2"
                      >
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-3">
                    {["instagram", "whatsapp", "web", "email", "otros"].map(
                      (c) => (
                        <label
                          key={c}
                          className="inline-flex items-center gap-2 rounded-lg border border-[var(--color-border)] px-3 py-2"
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div>
                  ...
                  >
                    ...
                    <option value="mensual-350-700">
                      Mensual 350–700 € (Care)
                    </option>
                  </select>
                </div>
                <div>
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  className="bg-bg mt-1 w-full rounded-lg border border-[var(--color-border)] px-3 py-2"
                  placeholder="Objetivos, contexto y lo imprescindible para arrancar."
                />
        ...
        <aside className="md:col-span-2">
          <Card className="sticky top-24 p-6 md:p-8">
            <h2 className="font-display mb-2 text-xl">Tu selección</h2>
            <p className="text-muted mb-4 text-sm">
              Producto: <span className="font-medium">{product?.title}</span>
              <br />
            ...
            <ul className="text-muted list-disc pl-4 text-sm">
              <li>{eta}</li>
            </ul>
            <div className="text-muted mt-4 text-xs">
              Tras recibir tu solicitud, te enviaremos una propuesta con alcance
              y agenda de arranque.
            </div>
          </Card>
        </aside>
      </div>
